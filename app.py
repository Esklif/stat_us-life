...
DATA_FILE = APP_DIR / "fandom_data.json"
BACKUP_FILE = APP_DIR / "fandom_data.json.bak"
DATA_VERSION = 2
...
def migrate_data():
    ...
    Старые миры, персонажи, посты и личные сообщения сохраняются.
    """
    ...
    for world in st.session_state["worlds"]:
        ...
        world["characters"] = [
            item for item in world["characters"] if isinstance(item, dict)
        ]
        world["feed"] = [
            item for item in world["feed"] if isinstance(item, dict)
        ]

        # Отдельный профиль игрока внутри мира
        ...
        player_profile.setdefault(
            "name",
            global_profile.get("name", "Игрок")
        )
        player_profile.setdefault(
            "avatar",
            global_profile.get("avatar")
        )
        player_profile.setdefault("bio", "")
        player_profile.setdefault("followers", 0)
        player_profile.setdefault("reposts", [])

        # Добавляем ID персонажам
        ...
        # Переносим старые переписки, которые хранились по имени
        ...
        # Добавляем недостающие поля постам
...
def call_api(messages, max_tokens=180):
    """
    Тихий вызов API.
    Не показывает спиннеры и ошибки в интерфейсе.
    """
...
def build_social_system_prompt(character, world):
    ...
    return (
        ...
        f"Твоя личность, характер и предыстория: "
...
def is_interested(character, world, content, content_type="пост"):
    """
    Персонаж самостоятельно решает, интересна ли ему публикация.
    """
    ...
    result = call_api(
        [
            {
                "role": "system",
                "content": build_social_system_prompt(character, world)
            },
            {
                "role": "user",
                "content": prompt
...
def generate_character_post(character, world, trigger_post):
    ...
    prompt = (
        ...
        f"Публикация, вызвавшая реакцию:\n"
        f"{trigger_post.get('author', 'Неизвестно')}: "
        f"{trigger_post.get('text', '')}\n\n"

        f"Последние публикации в ленте:\n"
        f"{feed_context or 'Лента пуста.'}\n\n"
...
def generate_character_comment(
    character,
    world,
    post,
    existing_comments,
    ...
    comments_context = "\n".join(
        f"{comment.get('author', 'Неизвестно')}: "
        f"{comment.get('text', '')}"
...
def generate_dm_reply(character, world, history):
    messages = [
        {
            "role": "system",
            "content": build_dm_system_prompt(character, world)
        }
    ]
...
def auto_comment_on_character_post(author_character, world, post):
    ...
    # Не более двух проверок интереса, чтобы не создавать слишком много запросов
    for candidate in candidates[:2]:
        interest_text = (
            f"Автор поста: {post['author']}\n"
        ...
        if is_interested(
            ...
            world,
            interest_text,
            content_type="пост другого персонажа"
        ):
...
def generate_reactions_to_user_post(world, user_post):
    """
    Заинтересованные персонажи публикуют собственные посты.
    Затем другой заинтересованный персонаж может прокомментировать их.
    """
    ...
    for character in characters[:MAX_REACTION_CANDIDATES]:
        if not is_interested(
            ...
            world,
            user_post["text"],
            content_type="пост игрока"
        ):
            continue

        generated_text = generate_character_post(
            character,
            world,
            user_post
        )
        ...
        world["feed"].insert(0, new_post)

        auto_comment_on_character_post(
...
def generate_reactions_to_user_comment(
    world,
    post,
    user_comment
):
    ...
    # Максимум два персонажа отвечают на комментарий пользователя
    reactions_count = 0
    ...
    for character in characters[:MAX_REACTION_CANDIDATES]:
        ...
        if not is_interested(
            ...
            world,
            interest_text,
            content_type="комментарий игрока"
        ):
            continue
...
st.markdown(
    ...
    <style>
        ...
        div[data-testid="stVerticalBlock"]:has(.avatar-upload-marker)
        div[data-testid="stFileUploader"] {
            width: 116px !important;
            height: 116px !important;
            margin: -116px auto 1.25rem auto !important;
            padding: 0 !important;
            opacity: 0;
            position: relative;
            z-index: 20;
        ...
        div[data-testid="stVerticalBlock"]:has(.avatar-upload-marker):hover
        .avatar-image,
        div[data-testid="stVerticalBlock"]:has(.avatar-upload-marker):hover
        .avatar-default {
        ...
        div.stFormSubmitButton > button[kind="primaryFormSubmit"]:hover {
            background-color: #4444DD !important;
            border-color: #4A4A4A !important;
            color: white !important;
        }
...
def render_feed(world):
    ...
    with st.container(border=True):
        ...
        if submitted and user_text.strip():
            ...
            # Запросы проходят без st.spinner и служебных надписей
...
def render_dm_list(world):
    st.markdown(
        '<span class="right-sidebar-marker"></span>',
        unsafe_allow_html=True
    )
