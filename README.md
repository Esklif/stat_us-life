...
`StatUsLife-Source-A.0.3.3.zip` содержит исходный код, сценарии установки и конфигурации сборки. В архив намеренно не включаются EXE, DLL, APK, PYC, виртуальные окружения, каталоги сборки, пользовательские данные, секреты и локальные пути Android SDK.
...
1. Скачайте `StatUsLife-Setup.exe` из релиза A.0.3.3.
...
Ключ не записывается в `fandom_data.json`. В Windows-приложении он хранится в защищённом хранилище учётных данных операционной системы, а на Android — в зашифрованных preferences. Для внешних API рекомендуется HTTPS: при использовании HTTP ключ и содержимое запросов могут быть перехвачены в сети.
...
Workflow `.github/workflows/release.yml` запускается для тегов `A.*`, сохраняет совместимость с тегами `v*` и может быть запущен вручную. Значение по умолчанию — `A.0.3.3`.

Для релиза A.0.3.3:

```powershell
git tag A.0.3.3
git push origin A.0.3.3
```
...
- `StatUsLife-Source-A.0.3.3.zip` без скомпилированных файлов;
...
- `installer.iss` — конфигурация Inno Setup версии A.0.3.3.
