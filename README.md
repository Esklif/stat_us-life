...
`StatUsLife-Source-A.0.3.1.zip` содержит исходный код, сценарии установки и конфигурации сборки. В архив намеренно не включаются EXE, DLL, APK, PYC, виртуальные окружения, каталоги сборки, пользовательские данные, секреты и локальные пути Android SDK.
...
1. Скачайте `StatUsLife-Setup.exe` из релиза A.0.3.1.
...
Workflow `.github/workflows/release.yml` запускается для тегов `A.*`, сохраняет совместимость с тегами `v*` и может быть запущен вручную. Значение по умолчанию — `A.0.3.1`.

Для релиза A.0.3.1:

```powershell
git tag A.0.3.1
git push origin A.0.3.1
```
...
- `StatUsLife-Source-A.0.3.1.zip` без скомпилированных файлов;
...
- `installer.iss` — конфигурация Inno Setup версии A.0.3.1.
